...
export function CatalogoOlfativoClient() {
  ...
  return (
    <main className="min-h-screen bg-[#050505] text-stone-100">
      ...
      <section className="border-y border-white/10 bg-[#090806] px-6 py-16 sm:px-10 lg:px-12"><div className="mx-auto max-w-7xl"><p className="text-xs font-semibold uppercase tracking-[.3em] text-gold">Curadoria rápida</p><h2 className="mt-4 text-3xl font-semibold text-white">Não sabe qual escolher?</h2><p className="mt-3 text-sm text-stone-400">Conte como você quer se sentir e veja uma seleção compatível.</p><div className="mt-8 grid grid-cols-2 gap-3 md:grid-cols-3">{quickChoices.map(([value, label]) => <button key={label} type="button" onClick={() => applyChoice(value)} className="min-h-16 border border-gold/20 bg-gold/[.06] p-4 text-sm font-semibold text-gold-light transition hover:border-gold/60 hover:bg-gold/15">{label}</button>)}</div></div></section>
